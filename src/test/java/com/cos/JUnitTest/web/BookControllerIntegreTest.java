...
public class BookControllerIntegreTest {
	...
	public void save_테스트() throws Exception {
		// given (테스트를 하기 위한 준비)
		Book book = new Book(1, "제목입니다",3.0 ,3.0);
		...
		// then (검증)
		resultAction.andExpect(status().isCreated()).andExpect(jsonPath("$.title").value("제목입니다"))
	...
	public void findAll_테스트() throws Exception {
		...
		books.add(new Book(1, "제목입니다1",3.0 ,3.0));
		books.add(new Book(2, "제목입니다2",3.5 ,3.5));
		books.add(new Book(3, "제목입니다3",4.0 ,4.0));
		...
		resultAction.andExpect(status().isOk()).andExpect(jsonPath("$.[0].id").value(1L))
				.andExpect(jsonPath("$", Matchers.hasSize(3))).andExpect(jsonPath("$.[2].title").value("제목입니다3"))
				.andDo(MockMvcResultHandlers.print());
	}
	...
	public void findById_테스트() throws Exception {
		...
		// then
		resultAction.andExpect(status().isOk()).andExpect(jsonPath("$.title").value("제목입니다2"))
				.andDo(MockMvcResultHandlers.print());
	}
